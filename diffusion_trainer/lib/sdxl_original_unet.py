"""U-Net of sd_xl_base based on Diffusers code.

The format of the state dict has been adjusted to match SDXL.
Edit From `kohya-ss/sd-scripts`.
    ...
    def forward(ctx, q, k, v, mask, causal, q_bucket_size, k_bucket_size):
        ...
        for ind, (qc, oc, row_mask, row_sums, row_maxes) in enumerate(row_splits):
            ...
            for k_ind, (kc, vc) in enumerate(col_splits):
                ...
                block_row_sums = exp_weights.sum(dim=-1, keepdim=True).clamp(min=EPSILON)
...
def get_parameter_dtype(parameter: torch.nn.Module) -> torch.dtype:
    """Get the dtype of the first parameter of a module."""
...
def get_timestep_embedding(
    ...
    """Get positional embeddings for a set of timesteps.
    ...
    if len(timesteps.shape) == 1:
        msg = "Timesteps should be a 1d-array"
...
def resize_like(x: torch.Tensor, target: torch.Tensor, mode: str = "bicubic", *, align_corners: bool = False) -> torch.Tensor:
    """Resize a tensor to match the shape of a target tensor."""
    org_dtype = x.dtype
...
class GroupNorm32(nn.GroupNorm):
    """GroupNorm torch.float32 version."""
...
class ResnetBlock2D(nn.Module):
    ...
    def forward_body(self, x: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        """Forward method body."""
        h = self.in_layers(x)
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        ...
        if self.training and self.gradient_checkpointing:
            ...
            def create_custom_forward(func: Callable) -> Callable:
...
class Downsample2D(nn.Module):
    ...
    def forward_body(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if hidden_states.shape[1] != self.channels:
            msg = f"hidden_states.shape[1] != self.channels: {hidden_states.shape[1]} != {self.channels}"
            raise ValueError(msg)
        return self.op(hidden_states)
...
class CrossAttention(nn.Module):
    ...
    def set_use_sdpa(self, *, use_sdpa: bool) -> None:
    ...
    def forward(self, hidden_states: torch.Tensor, context: torch.Tensor | None = None, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # linear proj
        # return self.to_out[1](hidden_states)     # no dropout
    ...
    def forward_memory_efficient_xformers(
        self,
        x: torch.Tensor,
    ...
    def forward_memory_efficient_mem_eff(
        self,
        x: torch.Tensor,
        context: torch.Tensor | None = None,
...
class BasicTransformerBlock(nn.Module):
    ...
    def set_use_memory_efficient_attention(self, *, xformers: bool, mem_eff: bool) -> None:
        self.attn1.set_use_memory_efficient_attention(xformers=xformers, mem_eff=mem_eff)
        self.attn2.set_use_memory_efficient_attention(xformers=xformers, mem_eff=mem_eff)

    def set_use_sdpa(self, *, use_sdpa: bool) -> None:
        self.attn1.set_use_sdpa(use_sdpa=use_sdpa)
        self.attn2.set_use_sdpa(use_sdpa=use_sdpa)
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if not isinstance(output, torch.Tensor):
                msg = "output is not torch.Tensor"
                raise ValueError(msg)
        else:
...
class Transformer2DModel(nn.Module):
    def __init__(
        ...
        cross_attention_dim: int | None = None,
        use_linear_projection: bool = False,  # noqa: FBT001, FBT002
...
class SdxlUNet2DConditionModel(nn.Module):
    ...
    def set_attention_slice(self) -> Never:
        msg = "Attention slicing is not supported for this model."
...
class InferSdxlUNet2DConditionModel:
    ...
    def set_deep_shrink(self, ds_depth_1, ds_timesteps_1=650, ds_depth_2=None, ds_timesteps_2=None, ds_ratio=0.5):
        if ds_depth_1 is None:
            ...
            logger.info(
                f"Deep Shrink is enabled: [depth={ds_depth_1}/{ds_depth_2}, timesteps={ds_timesteps_1}/{ds_timesteps_2}, ratio={ds_ratio}]",  # noqa: G004
            )
    ...
    def forward(
        ...
        # assert x.dtype == y.dtype, f"dtype mismatch: {x.dtype} != {y.dtype}"
        if x.dtype != y.dtype:
            msg = f"dtype mismatch: {x.dtype} != {y.dtype}"
            raise ValueError(msg)
        ...
        h = h.type(x.dtype)
        return call_module(_self.out, h, emb, context)
...
if __name__ == "__main__":
    ...
    unet.to("cuda")
    unet.set_use_memory_efficient_attention(xformers=True, mem_eff=False)
    ...
    steps = steps - 1
    logger.info("elapsed time: %f [sec] for last %d steps", elapsed_time, steps)
